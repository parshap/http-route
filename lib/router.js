 ...
 *     // `/foo` (e.g., `/foo` or `/foo/bar`) with any method (e.g., GET,
 *     // POST).
 *     res.end("foo");
 * });
 *
...
module.exports = function createRouter() {
	...
	app.mount = function(condition, app) {
		return this.use(createRoute(condition, app));
	};

	app.route = function(condition, app) {
		var handle = createMiddleware(app);
		return this.use(createRoute(condition, function(req, res, next) {
			if (req.url !== "/") {
				return next();
			}
